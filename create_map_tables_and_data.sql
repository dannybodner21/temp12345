...
-- Create service_categories table (referenced in other components)
...
-- Insert test service providers in Los Angeles
...
CROSS JOIN (
    VALUES 
        ('Deep Tissue Massage', 120.00, 90),
        ('Swedish Massage', 95.00, 60),
        ('Hot Stone Massage', 140.00, 90),
        ('Facial Treatment', 85.00, 60),
        ('Anti-Aging Facial', 115.00, 75),
        ('Manicure', 45.00, 45),
        ('Pedicure', 55.00, 60),
        ('Gel Manicure', 60.00, 60),
        ('Hair Cut & Style', 75.00, 60),
        ('Hair Color', 150.00, 120),
        ('Yoga Session', 35.00, 60),
        ('Meditation Class', 25.00, 45)
) AS services_data(service_name, price, duration)
WHERE sp.business_name IN (
    CASE 
        WHEN service_name LIKE '%Massage%' THEN 'Beverly Hills Spa & Wellness'
        WHEN service_name LIKE '%Massage%' THEN 'Santa Monica Beach Massage'
        WHEN service_name LIKE '%Facial%' THEN 'Hollywood Beauty Studio'
        WHEN service_name LIKE '%Manicure%' OR service_name LIKE '%Pedicure%' THEN 'West Hollywood Nail Lounge'
        WHEN service_name LIKE '%Hair%' THEN 'Hollywood Beauty Studio'
        WHEN service_name LIKE '%Yoga%' OR service_name LIKE '%Meditation%' THEN 'Downtown LA Wellness Center'
        ELSE sp.business_name
    END
);

-- Simplified service insertion for each provider
DELETE FROM public.services; -- Clear any existing services first

-- Beverly Hills Spa & Wellness (Massage services)
INSERT INTO public.services (provider_id, name, price, duration_minutes, is_available)
...
-- Santa Monica Beach Massage
INSERT INTO public.services (provider_id, name, price, duration_minutes, is_available)
SELECT id, 'Hot Stone Massage', 140.00, 90, true FROM public.service_providers WHERE business_name = 'Santa Monica Beach Massage'
UNION ALL
SELECT id, 'Swedish Massage', 85.00, 60, true FROM public.service_providers WHERE business_name = 'Santa Monica Beach Massage';

-- Hollywood Beauty Studio (Facial & Hair)
INSERT INTO public.services (provider_id, name, price, duration_minutes, is_available)
...
SELECT 
    s.id,
    CURRENT_DATE + interval '0 days' as date,
    time_slot,
    true
